function [ ] = PlotS1(user1_successRate, user3_successRate, user1_rejectedRate, ... 
    user3_rejectedRate,user1_timeOutRate, user3_timeOutRate, RQNo, tech)
    
    group1 = [repmat({'1: DoS'}, length(user1_successRate), 1); ... 
    repmat({'2: Regular'}, length(user3_successRate), 1); ....
    repmat({'3: DoS'}, length(user1_rejectedRate), 1); ....
    repmat({'4: Regular'}, length(user3_rejectedRate), 1); ....
    repmat({'5: DoS'}, length(user1_timeOutRate), 1); ....
    ...
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-boxplotData.csv',RQNo, tech),csvinput);
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-%d.csv',RQNo, tech, 1),user1_successRate);
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-%d.csv',RQNo, tech, 2),user3_successRate);
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-%d.csv',RQNo, tech, 3),user1_rejectedRate);
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-%d.csv',RQNo, tech, 4),user3_rejectedRate);
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-%d.csv',RQNo, tech, 5),user1_timeOutRate);
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-%d.csv',RQNo, tech, 6),user3_timeOutRate);
    ...
    csvwrite(sprintf('RQ-Output\\RQ%d-S1-%s-errorBarData.csv',RQNo, tech),csvinput);
